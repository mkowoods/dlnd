...
#Changes to make this work with python3.7\6
xrange = range
ENCODING = 'ISO-8859-1'
...
def question_answers(id2line, convos):
    ...
    print('len id2line:', len(id2line), 'len convos:', len(convos))
    ...
    print('len q and a', len(questions))
...
def prepare_dataset(questions, answers):
    ...
    for filename in filenames:
        print('prepare_dataset:', filename)
        files.append(open(os.path.join(config.PROCESSED_PATH, filename),'w'))

    print('starting write test and training data set')
    #print('Num Samples:', )
    #writes the batches in a correlated way... since nearby values are writte sequentially
...
def build_vocab(filename, normalize_digits=True):
    ...
    with open(out_path, 'w') as f:
        ...
        for word in sorted_vocab:
            #once the word frequency falls below the threshold this stops the loop and updates the config.py file
...
def prepare_raw_data():
    ...
    print('Running id2line')
    id2line = get_lines()

    print('Running convos')
    convos = get_convos()

    print('gathering questions and answers')
    questions, answers = question_answers(id2line, convos)

    print('running prepare dataset')
